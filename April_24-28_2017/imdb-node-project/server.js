...
app.get('/', (req, res) => {
  ...
  res.render('pages/index', movieIndexObj)
})
...
app.get('/movie/:movieId', (req, res) => {
  ...
  res.render('pages/movie', indivMovieObj);
})

app.get('/search', (req, res) => {
  /* Search Results Page */
  let search = req.query;
  let gotmovies = getmovies();
  let movies = [];
  // console.log('this part is working');

  for (let i = 0; i < gotmovies.length; i++) {
    if ((gotmovies[i].title.toLowerCase().indexOf(search.titleSearch.toLowerCase())) > -1) {
      movies.push(gotmovies[i]);
  ...
  res.render('pages/search', { searchedMovies: movies })
})
